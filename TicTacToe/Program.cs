...
class TicTacToe
{
       ...
       static void Main()
       {
           ...
           int turn=1; //Отмечает ход игрока
           ...
           do
           {
               ...
               Console.WriteLine($"Ход {turn}: Игрок {player}, введите номер ячейки:");
               ...
               try // на случай ввода чего-либо, кроме целового числа. Иначе цикл выбора ячейка для текущего игрока повторяется
               {
                   ...
                   if (mark >= 1 & mark <= 9 & tab[mark - 1] != 'X' & tab[mark - 1] != '0') 
                       // Проверка корректности ввода (число от 1 до 9, на ячейка не должно быть Х или 0
                   {
                       Console.ResetColor();
                       tab[mark - 1] = (player == 1) ? 'X' : '0';
                       // Изменение ячейки массива на Х или 0 в зависимости от текущего игрока 
                       Console.Clear();
                       Tabcreate();
                       turn++;
